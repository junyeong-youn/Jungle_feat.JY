...
int mm_init(void)
{
    if ((heap_listp = mem_sbrk(8 * WSIZE)) == (void *)-1) //?이건 잘 모르겟당
        return -1;
    PUT(heap_listp, PACK(0, 1));
    PUT(heap_listp + (1 * WSIZE), PACK(MINFREESIZE, 0));
    PUT(heap_listp + (2 * WSIZE), PACK(0, 0));
    PUT(heap_listp + (3 * WSIZE), PACK(0, 0));
    PUT(heap_listp + (4 * WSIZE), PACK(MINFREESIZE, 0));
