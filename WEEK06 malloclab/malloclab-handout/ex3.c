...
int mm_init(void)
{
    if((heap_listp = mem_sbrk(6*WSIZE)) == (void*)-1)
        return -1;
    PUT(heap_listp, 0);
    PUT(heap_listp + (1*WSIZE), PACK(MINFREE, 1)); // 헤더
    PUT(heap_listp + (2*WSIZE), NULL); // 프리브
    PUT(heap_listp + (3*WSIZE), NULL);      //넥스트
    PUT(heap_listp + (4*WSIZE), PACK(MINFREE, 1)); //푸터
    PUT(heap_listp + (5*WSIZE), PACK(0, 1)); //블랭크
...
static void* coalesce(void *bp)
{
    ...
    if (prev_alloc && next_alloc)
        if(NEXT_FREE(heap_listp) == NULL){
            NEXT_FREE(heap_listp) = bp + WSIZE;
            PREV_FREE(bp) = PREV_FREE(heap_listp);
        }
        else{
